    ...
    m_matrix(matrix), m_localP0(p0), m_localP1(p1)
{
  ...
  for (dgInt32 i = 0; i < 3; i++)
  {
    for (dgInt32 j = 0; j < 3; j++)
    {
      ...
      d = m_size.m_x * dgAbsf(axis % m_matrix[0])
          + m_size.m_y * dgAbsf(axis % m_matrix[1])
          + m_size.m_z * dgAbsf(axis % m_matrix[2]) + dgFloat32(1.0e-3f);
      c = origin % axis;
    ...
    if (dgOverlapTest(p0, p1, otherNode->m_p0, otherNode->m_p1))
    {
      for (dgInt32 i = 0; i < 3; i++)
      {
        for (dgInt32 j = 0; j < 3; j++)
        {
          ...
          d = m_size.m_x * axisAbs.m_x + m_size.m_y * axisAbs.m_y
              + m_size.m_z * axisAbs.m_z + dgFloat32(1.0e-3f);
          c = m_origin % axis;
          x0 = c - d;
          x1 = c + d;
          _ASSERTE(x0 <= x1);
          ...
          d = otherNode->m_size.m_x * axisDotAbs.m_x
              + otherNode->m_size.m_y * axisDotAbs.m_y
              + otherNode->m_size.m_z * axisDotAbs.m_z + dgFloat32(1.0e-3f);
          c = otherOrigin % axis;
          z0 = c - d;
          z1 = c + d;
          _ASSERTE(z0 <= z1);
    ...
    if (dgOverlapTest(p0, p1, data.m_localP0, data.m_localP1))
    {
      for (dgInt32 i = 0; i < 3; i++)
      {
        for (dgInt32 j = 0; j < 3; j++)
        {
          ...
          d = m_size.m_x * axis.m_x + m_size.m_y * axis.m_y
              + m_size.m_z * axis.m_z + dgFloat32(1.0e-3f);
...
dgCollisionCompound::dgCollisionCompound(dgWorld* world) :
    dgCollision(world->GetAllocator(), 0, dgGetIdentityMatrix(),
        m_compoundCollision)
{
  m_world = world;
  m_root = NULL;
...
dgCollisionCompound::dgCollisionCompound(dgInt32 count,
    dgCollisionConvex* const shapeArray[], dgWorld* world) :
    dgCollision(world->GetAllocator(), 0, dgGetIdentityMatrix(),
        m_compoundCollision)
{
  m_world = world;
...
dgCollisionCompound::dgCollisionCompound(const dgCollisionCompound& source) :
    dgCollision(source.GetAllocator(), 0, dgGetIdentityMatrix(),
        m_compoundCollision)
{
  int stack;
    ...
    dgVector& p1) const
{
  ...
  dgVector size(
      m_root->m_size.m_x * dgAbsf(matrix[0][0])
          + m_root->m_size.m_y * dgAbsf(matrix[1][0])
          + m_root->m_size.m_z * dgAbsf(matrix[2][0]) + DG_MAX_COLLISION_PADDING,
      m_root->m_size.m_x * dgAbsf(matrix[0][1])
          + m_root->m_size.m_y * dgAbsf(matrix[1][1])
          + m_root->m_size.m_z * dgAbsf(matrix[2][1]) + DG_MAX_COLLISION_PADDING,
      m_root->m_size.m_x * dgAbsf(matrix[0][2])
          + m_root->m_size.m_y * dgAbsf(matrix[1][2])
          + m_root->m_size.m_z * dgAbsf(matrix[2][2]) + DG_MAX_COLLISION_PADDING,
      dgFloat32(0.0f));
...
#define DG_MIN_VOLUME	(DG_MIN_SIDE * DG_MIN_SIDE * DG_MIN_SIDE) 
  ...
  inertia.m_x = totalInertia.m_x
      - (origin.m_y * origin.m_y + origin.m_z * origin.m_z);
  inertia.m_y = totalInertia.m_y
      - (origin.m_z * origin.m_z + origin.m_x * origin.m_x);
  inertia.m_z = totalInertia.m_z
      - (origin.m_x * origin.m_x + origin.m_y * origin.m_y);
    ...
    if (count > 2)
    {
      ...
      do
      {
        for (; i0 <= i1; i0++)
        {
          ...
          dgFloat32 val = (proxy->m_p0[axis] + proxy->m_p1[axis])
              * dgFloat32(0.5f);
          if (val > test)
          {
            break;
...
//					proxy.m_floatingMatrix = otherBody->m_collisionWorldMatrix;
          ...
          if (contactCount
              > (DG_MAX_CONTATCS - 2 * (DG_CONSTRAINT_MAX_ROWS / 3)))
          {
...
//	dgInt32 lru = m_world->m_broadPhaseLru;
  ...
  while (stack)
  {
    ...
    nodeProxi.m_origin = (nodeProxi.m_p1 + nodeProxi.m_p0).Scale(
        dgFloat32(0.5f));
