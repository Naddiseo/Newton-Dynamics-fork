...
dgCollisionConvexHull::dgCollisionConvexHull(dgMemoryAllocator* const allocator,
    ...
    dgFloat32 tolerance, const dgFloat32* vertexArray, const dgMatrix& matrix) :
    ...
    if (edge0->m_incidentFace != -1)
    {
      ...
      if (edge0->m_mark < mark)
      {
        ...
        if (test > dgFloat64(0.99995f))
        {
          ...
          if ((edge0->m_twin->m_next->m_twin->m_next != edge0)
              && (edge0->m_next->m_twin->m_next != edge0->m_twin))
          {
#define DG_MAX_EDGE_ANGLE dgFloat32 (1.0e-3f)
            ...
            dgBigVector e1(
                hullVertexArray[edge0->m_twin->m_next->m_next->m_incidentVertex]
                    - hullVertexArray[edge0->m_incidentVertex]);
            dgBigVector e0(
                hullVertexArray[edge0->m_incidentVertex]
                    - hullVertexArray[edge0->m_prev->m_incidentVertex]);
            ...
            if (projection >= DG_MAX_EDGE_ANGLE)
            {
              ...
              dgBigVector e1(
                  hullVertexArray[edge0->m_next->m_next->m_incidentVertex]
                      - hullVertexArray[edge0->m_twin->m_incidentVertex]);
              dgBigVector e0(
                  hullVertexArray[edge0->m_twin->m_incidentVertex]
                      - hullVertexArray[edge0->m_twin->m_prev->m_incidentVertex]);
              _ASSERTE((e0 % e0) >= dgFloat64 (0.0f));
              _ASSERTE((e1 % e1) >= dgFloat64 (0.0f));
