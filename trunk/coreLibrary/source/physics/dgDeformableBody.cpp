...
#if 0
void dgDeformableBody::SetAparentMassMatrix (const dgVector& massMatrix)
{
...
//	_ASSERTE (mass > dgFloat32 (0.0f));
	...
	if (mass >= DG_INFINITE_MASS) {
		...
		dgDeformableBodyMasterList& masterList (*m_world);
...
#ifdef _DEBUG
	dgDeformableBodyMasterList& me = *m_world;
	dgDeformableBodyMasterList::dgListNode* refNode;
	for (refNode = me.GetFirst(); refNode; refNode = refNode->GetNext()) {
		if (refNode->GetInfo().GetBody()->m_invMass.m_w != 0.0f) {
			for (; refNode; refNode = refNode->GetNext()) {
				dgDeformableBody* body = refNode->GetInfo().GetBody();
...
void dgDeformableBody::AttachCollision (dgCollision* collision)
{
	_ASSERTE (collision);
...
void dgDeformableBody::SetMatrixIgnoreSleep(const dgMatrix& matrix)
{
...
void dgDeformableBody::UpdateCollisionMatrixSimd (dgFloat32 timestep, dgInt32 threadIndex)
{
...
void dgDeformableBody::UpdateCollisionMatrix (dgFloat32 timestep, dgInt32 threadIndex)
{
...
/*
dgFloat32 dgDeformableBody::RayCast (
...
dgFloat32 dgDeformableBody::RayCastSimd (const dgLineBox& line, OnRayCastAction filter, OnRayPrecastAction preFilter, void* const userData, dgFloat32 minT) const
{
	_ASSERTE (filter);
	if (dgOverlapTestSimd (line.m_boxL0, line.m_boxL1, m_minAABB, m_maxAABB)) {
		dgContactPoint contactOut;
...
dgFloat32 dgDeformableBody::RayCast (const dgLineBox& line, OnRayCastAction filter, OnRayPrecastAction preFilter, void* const userData, dgFloat32 minT) const
{
	_ASSERTE (filter);
	if (dgOverlapTest (line.m_boxL0, line.m_boxL1, m_minAABB, m_maxAABB)) {
		dgContactPoint contactOut;
...
void dgDeformableBody::AddBuoyancyForce (
...
//			_ASSERTE (dgSqrt (torque % torque) < (dgSqrt (gravityVector % gravityVector) * m_mass.m_w * dgFloat32 (100.0f) * dgFloat32 (10.0f)));
			...
			m_world->GetIndirectLock(&m_locks);
			m_accel += force;
			m_alpha += torque;
			m_world->ReleaseIndirectLock(&m_locks);
 		}
	}
}
...
//void dgDeformableBody::CalcInvInertiaMatrix (dgMatrix& matrix) const
void dgDeformableBody::CalcInvInertiaMatrix ()
{
...
void dgDeformableBody::CalcInvInertiaMatrixSimd ()
{
	dgMatrix tmp;
...
/*
void dgDeformableBody::IntegrateVelocity (dgFloat32 timestep, bool update)
{
	dgFloat32 omegaMag2;
...
void dgDeformableBody::UpdateMatrix (dgFloat32 timestep, dgInt32 threadIndex)
{
	if (m_matrixUpdate) {
...
void dgDeformableBody::IntegrateVelocity (dgFloat32 timestep)
{
...
void dgDeformableBody::CalculateContinueVelocity (dgFloat32 timestep, dgVector& veloc, dgVector& omega) const
{
...
void dgDeformableBody::CalculateContinueVelocitySimd (
	dgFloat32 timestep,
	dgVector& veloc, 
...
dgVector dgDeformableBody::GetTrajectory (const dgVector& velocParam, const dgVector& omegaParam) const
{
_ASSERTE (0);
...
/*
void dgDeformableBody::IntegrateNotUpdate (dgFloat32 timestep)
{
	dgFloat32 omegaMag2;
	dgFloat32 omegaAngle;
	dgFloat32 invOmegaMag;
...
dgVector dgDeformableBody::CalculateInverseDynamicForce (const dgVector& desiredVeloc, dgFloat32 timestep) const
{
	//dgWord* world; 
	dgFloat32 massAccel;
...
dgConstraint* dgDeformableBody::GetFirstJoint() const
{
	dgDeformableBodyMasterListRow::dgListNode* node; 	
...
dgConstraint* dgDeformableBody::GetNextJoint(dgConstraint* joint) const
{
	dgDeformableBodyMasterListRow::dgListNode* node; 	
...
dgConstraint* dgDeformableBody::GetFirstContact() const
{
	dgDeformableBodyMasterListRow::dgListNode* node; 	
...
dgConstraint* dgDeformableBody::GetNextContact(dgConstraint* joint) const
{
	dgDeformableBodyMasterListRow::dgListNode* node; 	
...
void dgDeformableBody::SetFreeze (bool state)
{
	if (state){
		Freeze();
	} else {
...
void dgDeformableBody::Freeze ()
{
	if (m_invMass.m_w > dgFloat32 (0.0f)) {
		if (!m_freeze) {
			m_freeze = true;
			for (dgDeformableBodyMasterListRow::dgListNode* node = m_masterNode->GetInfo().GetFirst(); node; node = node->GetNext()) {
				dgDeformableBody* const body = node->GetInfo().m_bodyNode;
				body->Freeze ();
			}
		}
	}
...
void dgDeformableBody::Unfreeze ()
{
	if (m_invMass.m_w > dgFloat32 (0.0f)) {
...
dgMatrix dgDeformableBody::CalculateInertiaMatrix () const
{
	dgMatrix tmpMatrix;
...
dgMatrix dgDeformableBody::CalculateInvInertiaMatrix () const
{
	dgMatrix tmpMatrix;
...
void dgDeformableBody::AddImpulse (const dgVector& pointDeltaVeloc, const dgVector& pointPosit)
{
//	dgMatrix tmpMatrix;
//	tmpMatrix[0][0] = m_invMass[0] * m_matrix[0][0];
//	tmpMatrix[0][1] = m_invMass[1] * m_matrix[1][0];
...
void dgDeformableBody::ApplyImpulseArray (dgInt32 count, dgInt32 strideInBytes, const dgFloat32* const impulseArray, const dgFloat32* const pointArray)
{
...
void dgDeformableBody::InvalidateCache ()
{
	m_sleeping = false;
//	m_isInWorld = true;
	m_equilibrium = false;
