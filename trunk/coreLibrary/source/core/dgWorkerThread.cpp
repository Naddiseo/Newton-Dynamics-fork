...
void dgWorkerThread::Init (dgMemoryAllocator* const allocator, dgInt32 id, dgUnsigned32* const jobQueueCompletedSignal)
{
	_ASSERTE (!m_queue);
	...
	if (m_jobQueueCompletedSignal) {
		dgInterlockedExchange ((dgInt32*) m_jobQueueCompletedSignal, 0); 
	}
	m_queue = (dgWorkerThreadJob*) m_allocator->MallocLow (dgInt32 (m_jobsCapacity * sizeof (dgWorkerThreadJob)));  
...
void dgWorkerThread::RunAllMicroTask()
{
	...
	if (m_jobQueueCompletedSignal) {
		dgAtomicAdd ((dgInt32*) m_jobQueueCompletedSignal, 1); 
	}
}
...
void dgWorkerThread::TaskExecuter()
{
	...
	while (m_contineExecution) {
		...
		_ASSERTE (waitCode == WAIT_OBJECT_0); 
		RunAllMicroTask();
	}
...
/*
void dgWorkerThread::QueueJob (dgWorkerThreadTaskCallback callback, void** const userParamArray, dgInt32 paramCount)
{
	if (m_jobsCount == m_jobsCapacity) {
		dgWorkerThreadJob* const newQueue = (dgWorkerThreadJob*) m_allocator->MallocLow (dgInt32 (2 * m_jobsCapacity * sizeof (dgWorkerThreadJob)));  
	...
	for (dgInt32 i = 0; i < paramCount; i ++) {
