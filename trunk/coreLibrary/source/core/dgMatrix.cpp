...
static inline void ROT(dgMatrix &a, dgInt32 i, dgInt32 j, dgInt32 k, dgInt32 l,
    dgFloat32 s, dgFloat32 tau)
{
  dgFloat32 g = a[i][j];
  dgFloat32 h = a[k][l];
  a[i][j] = g - s * (h + g * tau);
...
void dgMatrix::EigenVectors(dgVector &eigenValues)
{
  ...
  for (dgInt32 i = 0; i < 50; i++)
  {
    ...
    {
      _ASSERTE(
          dgAbsf((eigenVectors.m_front % eigenVectors.m_front) - dgFloat32(1.0f)) < dgEPSILON);
      _ASSERTE(
          dgAbsf((eigenVectors.m_up % eigenVectors.m_up) - dgFloat32(1.0f)) < dgEPSILON);
      _ASSERTE(
          dgAbsf((eigenVectors.m_right % eigenVectors.m_right) - dgFloat32(1.0f)) < dgEPSILON);
    ...
    if ((i > 3) && (dgAbsf(d[0]) + g == dgAbsf(d[0]))
        && (dgAbsf(d[1]) + g == dgAbsf(d[1])))
    {
    ...
    else if (dgAbsf(mat[0][1]) > thresh)
    {
      ...
      else
      {
        ...
        t = dgFloat32(1.0f)
            / (dgAbsf(theta) + dgSqrt(dgFloat32(1.0f) + theta * theta));
    ...
    if ((i > 3) && (dgAbsf(d[1]) + g == dgAbsf(d[1]))
        && (dgAbsf(d[2]) + g == dgAbsf(d[2])))
    {
