...
class dgGlobalAllocator: public dgMemoryAllocator, public dgList<
    dgMemoryAllocator*>
{
public:
  dgGlobalAllocator() :
  ...
  dgInt32 GetMemoryUsed() const
  {
    ...
    for (dgList<dgMemoryAllocator*>::dgListNode* node = GetFirst(); node; node =
        node->GetNext())
    {
...
dgMemoryAllocator::dgMemoryAllocator()
{
  m_memoryUsed = 0;
  m_emumerator = 0;
  SetAllocatorsCallback(dgGlobalAllocator::m_globalAllocator.m_malloc,
      dgGlobalAllocator::m_globalAllocator.m_free);
...
void dgMemoryAllocator::SetAllocatorsCallback(dgMemAlloc memAlloc,
    dgMemFree memFree)
{
  m_free = memFree;
  m_malloc = memAlloc;
...
{
  _ASSERTE(
      (sizeof (dgMemoryCacheEntry) + sizeof (dgInt32) + sizeof(dgInt32)) <= DG_MEMORY_GRANULARITY);
...
//			charPtr = bin->m_pool
      for (dgInt32 i = 0; i < count; i++)
      {
        ...
        dgMemoryInfo* const info = ((dgMemoryInfo*) (charPtr
            + DG_MEMORY_GRANULARITY)) - 1;
...
void dgMemoryAllocator::Free(void *retPtr)
{
  ...
  else
  {
    ...
    dgMemoryCacheEntry* const cashe = (dgMemoryCacheEntry*) (((char*) retPtr)
        - DG_MEMORY_GRANULARITY);
