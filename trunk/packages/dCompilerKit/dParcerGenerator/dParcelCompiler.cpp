...
void dParcelCompiler::MatchToken (Token token)
{
	if (m_token == token) {
		NextToken ();
	} else {
...
// id							: .
// id							: CHARACTER 
void dParcelCompiler::ParseDefinitions (FILE* const file, const char* const className)
{
...
// DefinitionExpression	: DefinitionBlock 
// DefinitionExpression	: DefinitionBlock DefinitionExpression | nothing
void dParcelCompiler::ParseDefinitionExpression (string& preheaderCode)
{
	for (ParseDefinitionBlock (preheaderCode); (m_token != m_end) && (m_token != m_delimiter); ParseDefinitionBlock (preheaderCode));
}

// DefinitionBlock		: m_comment
void dParcelCompiler::ParseDefinitionBlock (string& preheaderCode)
{
...
void dParcelCompiler::DefinitionsMap::AddDefinition (const char* const label, const char* const regularExpression)
{
	ExpandedNFA* const expression = new ExpandedNFA (regularExpression, this);
	Insert(expression, dCRC (label));
}

dParcelCompiler::ExpandedNFA* dParcelCompiler::DefinitionsMap::FindNDAExpresion (const char* const label) const
{
...
dParcelCompiler::ExpandedNFA::ExpandedNFA (const char* const regularExpression, DefinitionsMap* const map)
...
void dParcelCompiler::ExpandedNFA::PreProcessExpression (const char* const regularExpression)
{
	char buffer[1024];
...
bool dParcelCompiler::ExpandedNFA::IsOperator (int charater) const
{
...
bool dParcelCompiler::ExpandedNFA::CheckInsertConcatenation (int left, int right) const
{
...
void dParcelCompiler::ExpandedNFA::PushNFA (ExpandedNFA* const nfa, const char* const label)
{
...
void dParcelCompiler::ExpandedNFA::ShiftID()
{
	if (m_token == '{') {
		char label[256];
		int i = 0; 
		...
		dParcelCompiler::ExpandedNFA* const expresion = m_map->FindNDAExpresion (label);
