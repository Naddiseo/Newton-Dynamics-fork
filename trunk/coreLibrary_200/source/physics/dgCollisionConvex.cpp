...
dgCollisionConvex::dgCollisionConvex(dgMemoryAllocator* const allocator,
    ...
    dgCollision(allocator, signature, matrix, id), m_volume(dgFloat32(0.0f),
        dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(1.0f)), m_boxSize(
        dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(1.0f)), m_boxOrigin(
        dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(0.0f), dgFloat32(1.0f))
{
  ...
  for (dgUnsigned32 i = 0;
      i
          < sizeof(m_supportVertexStarCuadrant)
              / sizeof(m_supportVertexStarCuadrant[0]); i++)
  {
...
void dgCollisionConvex::SetVolumeAndCG()
{
  ...
  dgInt32 count = sizeof(m_supportVertexStarCuadrant)
      / sizeof(m_supportVertexStarCuadrant[0]);
    ...
    dgVector& p1) const
{
  ...
  dgVector size(
      m_boxSize.m_x * dgAbsf(matrix[0][0])
          + m_boxSize.m_y * dgAbsf(matrix[1][0])
          + m_boxSize.m_z * dgAbsf(matrix[2][0]) + DG_MAX_COLLISION_PADDING,
      m_boxSize.m_x * dgAbsf(matrix[0][1])
          + m_boxSize.m_y * dgAbsf(matrix[1][1])
          + m_boxSize.m_z * dgAbsf(matrix[2][1]) + DG_MAX_COLLISION_PADDING,
      m_boxSize.m_x * dgAbsf(matrix[0][2])
          + m_boxSize.m_y * dgAbsf(matrix[1][2])
          + m_boxSize.m_z * dgAbsf(matrix[2][2]) + DG_MAX_COLLISION_PADDING,
      dgFloat32(0.0f));
...
//	dgConvexMassData localData;
  ...
  if (capEdge)
  {
    ...
    do
    {
      ...
      faceVertex[count] = m_vertex[edge->m_vertex]
          - dp.Scale(test[edge->m_vertex] / (plane % dp));
      count++;
      if (count == 127)
      {
...
{
  const dgVector dir(direction.m_x, direction.m_y, direction.m_z,
      dgFloat32(0.0f));
  _ASSERTE(dgAbsf(dir % dir - dgFloat32 (1.0f)) < dgFloat32 (1.0e-3f));
  ...
  do
  {
    ...
    ptr = ptr->m_twin->m_next;
    maxCount--;
  } while ((ptr != edge) && maxCount);_ASSERTE(maxCount);
...
dgVector dgCollisionConvex::SupportVertexSimd(const dgVector& direction) const
{
  ...
  simd_type dot0 =
      simd_mul_add_v (simd_mul_add_v (simd_mul_v (dir_x, *(simd_type*) &m_multiResDir_sse[0]),
  ...
  simd_type entry =
      simd_or_v (simd_and_v(*(simd_type*) &m_index_0123, mask), simd_andnot_v (*(simd_type*) &m_index_4567, mask));

  dot1 = simd_move_hl_v (dot0, dot0);
  mask = simd_cmpgt_v(dot0, dot1);
  dot0 = simd_max_v (dot0, dot1);
  entry =
      simd_or_v (simd_and_v(entry, mask), simd_andnot_v (simd_move_hl_v (entry, entry), mask));

  mask =
      simd_cmpgt_s(dot0, simd_permut_v (dot0, dot0, PURMUT_MASK (3, 2, 1, 1)));

  dgInt32 index =
      simd_store_is (simd_or_v (simd_and_v(entry, mask), simd_andnot_v (simd_permut_v (entry, entry, PURMUT_MASK (3, 2, 1, 1)), mask)));
  dgConvexSimplexEdge* edge = m_supportVertexStarCuadrant[index];
  index = edge->m_vertex;

  simd_type dir =
      simd_set (direction.m_x, direction.m_y, direction.m_z, dgFloat32 (0.0f));
  _ASSERTE(m_vertex[edge->m_vertex].m_w == dgFloat32 (1.0f));

  simd_type side0 = simd_mul_v (*(simd_type*)&m_vertex[edge->m_vertex], dir);
  side0 =
      simd_add_s(simd_add_v (side0, simd_move_hl_v (side0, side0)), simd_permut_v (side0, side0, PURMUT_MASK (3,3,3,1)));
  ...
  do
  {
    ...
    simd_type side1 =
        simd_mul_v (*(simd_type*)&m_vertex[ptr->m_twin->m_vertex], dir);
    side1 =
        simd_add_s(simd_add_v (side1, simd_move_hl_v (side1, side1)), simd_permut_v (side1, side1, PURMUT_MASK (3,3,3,1)));
    ...
    const dgVector& normal, dgVector* const polygon) const
{
  ...
  dgInt8 buffer[8 * DG_MAX_VERTEX_CLIP_FACE
      * (sizeof(dgInt32) + sizeof(dgFloat32))];
...
//					if (side0 >= dgFloat32 (0.0f)) {
    ...
    } while ((ptr != firstEdge) && (maxCount < DG_CLIP_MAX_COUNT));_ASSERTE(
...
#define DG_VOL  (DG_AREA * DG_LEN)
  ...
  {
    if ((m_collsionId != m_convexHullCollision)
        || (((dgCollisionConvexHull*) this)->m_faceCount > 48))
    {
//			dgInt32 i;
//			dgInt32 i0;
//			dgInt32 i1;
