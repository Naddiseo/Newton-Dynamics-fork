  ...
  for (dgInt32 i = 0; i < 3; i++)
  {
    ...
    for (iter.Begin(); iter; iter++)
    {
      ...
      if (edge->m_mark < mark)
      {
        ...
        if ((normal.m_z >= (normal.m_x - dgFloat32(1.0e-4f)))
            && (normal.m_z >= (normal.m_y - dgFloat32(1.0e-4f))))
        {
          dgEdge* ptr = edge;
          do
          {
...
//				#endif
        ...
        dgFloat64 mag3 = n % n;
        _ASSERTE(
            mag3 > dgFloat64 (DG_MESH_EFFECT_PRECISION_SCALE_INV * DG_MESH_EFFECT_PRECISION_SCALE_INV));
#endif
      }
    }
...
void dgMeshEffect::BuildFromVertexListIndexList(dgInt32 faceCount,
    ...
    dgInt32 uv1StrideInBytes, const dgInt32* const uv1Index)
{
  ...
  for (int i = 0; i < vertexCount; i++)
  {
    ...
    AddVertex(
        dgBigVector(vertex[index + 0], vertex[index + 1], vertex[index + 2],
            vertex[index + 3]));
  ...
  m_atribCount = dgVertexListToIndexList(&m_attib[0].m_vertex.m_x,
      sizeof(dgVertexAtribute), sizeof(dgVertexAtribute) / sizeof(dgFloat64),
      m_atribCount, &attrIndexMap[0], DG_VERTEXLIST_INDEXLIST_TOL);
  ...
  while (hasFaces)
  {
    ...
    if (hasFaces)
    {
      ...
      for (int i = 0; i < vertexCount; i++)
      {
        ...
        AddVertex(
            dgBigVector(vertex[index + 0], vertex[index + 1], vertex[index + 2],
                dgFloat64(layerCount + layerCountBase)));
      }
    }
  }
...
void dgMeshEffect::GetMaterialGetIndexStream(dgIndexArray* handle,
    dgInt32 materialHandle, dgInt32* indexArray)
{
  dgInt32 index;
  dgInt32 textureID;
...
void dgMeshEffect::GetMaterialGetIndexStreamShort(dgIndexArray* handle,
    dgInt32 materialHandle, dgInt16* indexArray)
{
  dgInt32 index;
  dgInt32 textureID;
    ...
    dgFloat64 param) const
{
  ...
  attrEdge.m_vertex.m_x = attrEdge0.m_vertex.m_x * t0
      + attrEdge1.m_vertex.m_x * t1;
  attrEdge.m_vertex.m_y = attrEdge0.m_vertex.m_y * t0
      + attrEdge1.m_vertex.m_y * t1;
  attrEdge.m_vertex.m_z = attrEdge0.m_vertex.m_z * t0
      + attrEdge1.m_vertex.m_z * t1;
    ...
    const dgBigVector& srcPoint, dgEdge* const face) const
{
  ...
  for (dgInt32 i = 0; i < 4; i++)
  {
    ...
    do
    {
      ...
      if ((va > minError) && (vb > minError) && (vc > minError)
          && (va < maxError) && (vb < maxError) && (vc < maxError))
      {
        edge2 = ptr;
        ...
        dgBigVector normal(
            attr0.m_normal_x * alpha0 + attr1.m_normal_x * alpha1
                + attr2.m_normal_x * alpha2,
            attr0.m_normal_y * alpha0 + attr1.m_normal_y * alpha1
                + attr2.m_normal_y * alpha2,
            attr0.m_normal_z * alpha0 + attr1.m_normal_z * alpha1
                + attr2.m_normal_z * alpha2, dgFloat32(0.0f));
        normal = normal.Scale(dgFloat64(1.0f) / sqrt(normal % normal));

#ifdef _DEBUG
        dgBigVector testPoint(
            attr0.m_vertex.m_x * alpha0 + attr1.m_vertex.m_x * alpha1
                + attr2.m_vertex.m_x * alpha2,
            attr0.m_vertex.m_y * alpha0 + attr1.m_vertex.m_y * alpha1
                + attr2.m_vertex.m_y * alpha2,
            attr0.m_vertex.m_z * alpha0 + attr1.m_vertex.m_z * alpha1
                + attr2.m_vertex.m_z * alpha2, dgFloat32(0.0f));
        ...
        attribute.m_u0 = attr0.m_u0 * alpha0 + attr1.m_u0 * alpha1
            + attr2.m_u0 * alpha2;
        attribute.m_v0 = attr0.m_v0 * alpha0 + attr1.m_v0 * alpha1
            + attr2.m_v0 * alpha2;
        attribute.m_u1 = attr0.m_u1 * alpha0 + attr1.m_u1 * alpha1
            + attr2.m_u1 * alpha2;
        attribute.m_v1 = attr0.m_v1 * alpha0 + attr1.m_v1 * alpha1
            + attr2.m_v1 * alpha2;
  ...
  *right = NULL;
  ...
  for (iter.Begin(); iter;)
  {
    ...
    if (edge->m_mark != mark)
    {
      edge->m_mark = mark;
      edge->m_twin->m_mark = mark;
      if (vertexSide[edge->m_incidentVertex]
          * vertexSide[edge->m_twin->m_incidentVertex] < 0)
      {

        dgFloat64 test0 = mesh.m_points[edge->m_incidentVertex].m_w;

        dgBigVector dp(
            mesh.m_points[edge->m_twin->m_incidentVertex]
                - mesh.m_points[edge->m_incidentVertex]);
  ...
  for (iter.Begin(); iter; iter++)
  {
    ...
    if ((face->m_incidentFace > 0) && (face->m_mark != mark)
        && (vertexSide[face->m_incidentVertex] == 0)
        && (vertexSide[face->m_next->m_incidentVertex] < 0))
    {
      dgEdge* ptr = face;
      do
      {
  ...
  if (backMesh && frontMesh)
  {
    ...
    if (!(backMesh->PlaneApplyCap(&planeMesh, plane)
        && frontMesh->PlaneApplyCap(&planeMesh, plane.Scale(dgFloat32(-1.0f)))))
    {
      backMesh->Release();
      frontMesh->Release();
      *left = NULL;
      ...
      if (maxDist <= dgFloat32(1.5e-3f))
      {
        ...
        while (haveColinear)
        {
          ...
          do
          {
            ...
            {
              dgBigVector e0(
                  m_points[ptr->m_next->m_incidentVertex]
                      - m_points[ptr->m_incidentVertex]);
              dgBigVector e1(
                  m_points[ptr->m_next->m_next->m_incidentVertex]
                      - m_points[ptr->m_next->m_incidentVertex]);
      ...
      if (!hasCoplanar && ((backCount == 0) || (frontCount == 0)))
      {
        ...
        if (frontCount)
        {
          ...
          for (dgList<dgMeshTreeCSGFace*>::dgListNode* node1 =
              faceList.GetFirst(); node1; node1 = node1->GetNext())
          {
      ...
      if (isTJoint)
      {
        do
        {
          ...
          if (dist20 > dist10)
          {
            ...
            bool restart = false;
            if ((firstEdge->m_prev == &(*iter))
                || (firstEdge->m_prev->m_twin == &(*iter)))
            {
              restart = true;
            }
