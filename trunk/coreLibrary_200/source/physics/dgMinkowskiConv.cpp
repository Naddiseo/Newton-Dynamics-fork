...
class dgContactSolver
{
  ...
  {
    dgMinkError, dgMinkDisjoint, dgMinkIntersecting
  };
...
//				if (dist < dgFloat32 (5.0e-4f)) {
        if (dist < (DG_DISTANCE_TOLERANCE * dgFloat32(0.5f)))
        {
          ...
          case 2:
          {
            ...
            dgFloat32 alpha1 = -(m_hullVertex[0] % dp)
                / (dp % dp + dgFloat32(1.0e-24f));
...
//		dgVector* output;
    ...
    dgInt32 count = 0;
    dgVector* output = (dgVector*) &m_hullVertex[shape1VertexCount
        + shape2VertexCount + 1];
      ...
      dgInt32 contacID, dgContactPoint* const contactOut, dgInt32 maxContacts)
  {
    ...
    dgFloat32 dist = (plane % m_averVertex[face->m_vertex[0]])
        * dgFloat32(0.5f);
...
//			ptr = NULL;
      _ASSERTE(
          (shape1VertexCount + shape2VertexCount) < sizeof (subdivision) / (2 * sizeof (subdivision[0])));
      for (dgInt32 i0 = 0; i0 < shape2VertexCount; i0++)
      {
    ...
    if (count1)
    {
      ...
      if (count2)
      {
        ...
        dgInt32 edgeContactFlag =
            (m_floatingcollision->IsEdgeIntersection()
                | m_referenceCollision->IsEdgeIntersection()) ? 1 : 0;
        for (dgInt32 i = 0; i < count; i++)
        {
...
//		m_facePlaneIndex = 0;
    ...
    dgVector step(
        m_localRelVeloc.Scale(
            timestep
                + DG_ROBUST_PLANE_CLIP
                    * dgRsqrt(m_localRelVeloc % m_localRelVeloc)));
    for (dgInt32 i = 0; i < 3; i++)
    {
      m_hullVertex[i] = diffPoins[i] - step;
      m_averVertex[i] = averPoins[i] + step;
        ...
        i++)
    {
      ...
      if (dist < dgFloat32(1.0e-9f))
      {
        ...
        default:
        {
          ...
          if (dgAbsf((m_hullVertex[4] - p0) % normal)
              > dgAbsf((m_hullVertex[3] - p0) % normal))
          {
        ...
        i++)
    {
      ...
      if (dist < dgFloat64(1.0e-9f))
      {
        ...
        default:
        {
          ...
          if (fabs((m_hullVertexLarge[4] - p0) % normal)
              > fabs((m_hullVertexLarge[3] - p0) % normal))
          {
...
//						} while (keepSeaching);
        ...
        case 2:
        {
          ...
          } while (val < DG_FALLBACK_SEPARATING_DIST_TOLERANCE);_ASSERTE(
      ...
      if (faceIndex >= 0)
      {
        ...
        if (dist < DG_DISTANCE_TOLERANCE)
        {
          ...
          for (; i < 4; i++)
          {
            ...
            if ((error % error)
                < (DG_DISTANCE_TOLERANCE * DG_DISTANCE_TOLERANCE))
            {
              plane = face;
              //code = dgMinkDisjoint;
      ...
      const dgBigVector& origin)
  {
    ...
    for (; face && (j < DG_UPDATE_SEPARATING_PLANE_MAX_ITERATION); j++)
    {
      ...
      if (face)
      {
        ...
        dgFloat64 dist = normal
            % (m_hullVertexLarge[4] - m_hullVertexLarge[index]);
...
//		if (mag2 > DG_DISTANCE_TOLERANCE_ZERO) {
  ...
  dgMinkFace *CalculateClipPlaneSimd()
  {
    ...
    dgVector diff[3];
    dgVector aver[3];
    dgInt8 buffer[DG_HEAP_EDGE_COUNT
        * (sizeof(dgFloat32) + sizeof(dgMinkFace *))];
...
//						dgTrace (("Max face count overflow, breaking with last best face\n"));
        ...
        if (dist
            < (dgFloat32(DG_IMPULSIVE_CONTACT_PENETRATION) / dgFloat32(16.0f)))
        {
            ...
            for (i = 0; i < 3; i++)
            {
              ...
              if (adjacent->m_isActive)
              {
                ...
                if (dist > dgFloat32(0.0f))
                {
                  ...
                  _ASSERTE(
                      deadCount < sizeof (deadFaces) / sizeof (deadFaces[0]));
...
//						(!m_simplex[silhouette->m_adjancentFace[2]].m_isActive)) == 2));
          ...
          _ASSERTE(
              silhouetteCapCount < (sizeof (sillueteCap) / sizeof (sillueteCap[0])));
          do
          {
...
//							(!m_simplex[silhouette->m_adjancentFace[2]].m_isActive)) == 2));
          ...
          } while ((silhouette != lastSilhouette)
              || (silhouette->m_vertex[adjacentIndex ? adjacentIndex - 1 : 2]
                  != lastSilhouetteVertex));
...
public:
  inline dgContactSolver(dgCollisionParamProxy& proxy, dgCollision *polygon) :
...
//		 m_averVertexLarge ((dgBigVector*) m_averVertex)
  ...
  inline dgContactSolver(dgCollisionParamProxy& proxy) :
      m_matrix(*proxy.m_localMatrixInv)
//		 m_simplexLarge ((dgMinkFaceLarge*) m_simplex),
//		 m_hullVertexLarge ((dgBigVector*) m_hullVertex),
//		 m_averVertexLarge ((dgBigVector*) m_averVertex)
...
//		dgInt32 i0;
      ...
      dgInt32 conditionalContactCalculationAtOrigin)
  {
    ...
    else if (code == dgMinkDisjoint)
    {
      ...
      i2 = face->m_vertex[2];
      dgVector plane(
          (m_hullVertex[i1] - m_hullVertex[i0])
              * (m_hullVertex[i2] - m_hullVertex[i0]));
...
//			_ASSERTE (dgAbsf (plane % vRel) > dgFloat32 (0.0f));
      ...
      if (projVeloc >= dgFloat32(-1.0e-24f))
      {
        ...
        i1 = face->m_vertex[1];
        i2 = face->m_vertex[2];
        plane = (m_hullVertex[i1] - m_hullVertex[i0])
            * (m_hullVertex[i2] - m_hullVertex[i0]);
        _ASSERTE(plane % plane > dgFloat32 (0.0f));
        projVeloc = plane.DotProductSimd(m_localRelVeloc);
        if (projVeloc > dgFloat32(-1.0e-24f))
...
//			timeOfImpact = (plane % m_hullVertex[i0]) / (plane % m_localRelVeloc + dgFloat32 (1.0e-24f));
      timeOfImpact = (plane.DotProductSimd(m_hullVertex[i0]))
          / (plane.DotProductSimd(m_localRelVeloc) + dgFloat32(1.0e-24f));
...
//							contactOut[i0].m_point += step;
      ...
      dgInt32 conditionalContactCalculationAtOrigin)
  {
    ...
    else if (code == dgMinkDisjoint)
    {
      ...
      //			timeOfImpact = - plane.m_w / (plane % vRel + dgFloat32 (1.0e-24f));
      timeOfImpact = (plane % m_hullVertex[i0])
          / (plane % m_localRelVeloc + dgFloat32(1.0e-24f));
...
//		ptr[3] = -1;
  ...
  for (i = 0;
      i
          < dgInt32(
              sizeof(dgContactSolver::m_dir)
                  / sizeof(dgContactSolver::m_dir[0])); i++)
  {
                  ...
                  / sizeof(dgContactSolver::m_dir[0])); i++)
  {
    ...
    for (dgInt32 j = i + 1;
        j
            < dgInt32(
                sizeof(dgContactSolver::m_dir)
                    / sizeof(dgContactSolver::m_dir[0])); j++)
    {
      _ASSERTE(
    ...
    const dgVector& sph1, dgFloat32 radius1, dgCollisionParamProxy& proxy) const
{
  ...
  contactOut[0].m_point = sph0
      - contactOut[0].m_normal.Scale(mag * dgFloat32(0.5f));
  contactOut[0].m_penetration = dist;
  contactOut[0].m_userId = 0;
  return 1;
}
    ...
    dgCollisionParamProxy& proxy) const
{
  ...
  if (dgAbsf(dot) > dgFloat32(0.998f))
  {
    ...
    contactOut[0].m_point = cylP0 + p10.Scale(clip0 - pl0)
        - contactOut[0].m_normal.Scale(mag * dgFloat32(0.5f));
    contactOut[0].m_userId = 0;
    contactOut[0].m_penetration = dist;

    contactOut[1].m_normal = dir.Scale(mag);
    contactOut[1].m_point = cylP0 + p10.Scale(clip1 - pl0)
        - contactOut[1].m_normal.Scale(mag * dgFloat32(0.5f));
    ...
    dgCollisionParamProxy& proxy) const
{
  ...
  case 0 * 9 + 0 * 3 + 1:
  {
    ...
    normal.m_x = 1.0f;
    point.m_x = -size.m_x
        - (dist + proxy.m_penetrationPadding) * dgFloat32(0.5f);
    break;
  }
...
//	return mink.HullHullContactsSimd (0);
  if ((radiusA * dgFloat32(64.0f) < radiusB)
      || (radiusB * dgFloat32(64.0f) < radiusA))
  {
...
dgInt32 dgWorld::CalculateHullToHullContacts(dgCollisionParamProxy& proxy) const
{
  ...
  return mink.HullHullContactsLarge (0);
#else
  if ((radiusA * dgFloat64(32.0f) < radiusB)
      || (radiusB * dgFloat64(32.0f) < radiusA))
  {
    return mink.HullHullContactsLarge(0);
  }
  else
    ...
    dgCollisionParamProxy& proxy) const
{
  ...
  if (!(((dgCollisionConvex*) collision1)->m_vertexCount
      && ((dgCollisionConvex*) collision2)->m_vertexCount))
  {
    return count;
  }
  ...
  if (proxy.m_continueCollision)
  {
    ...
    if (count)
    {
      ...
      dist *= dist;
      distTravel = (mink.m_localRelVeloc % mink.m_localRelVeloc) * timestep
          * timestep;
    ...
    if (polygon->PointToPolygonDistance(center, radius, point, isEdge))
    {
      ...
      if (dist2 > dgFloat32(0.0f))
      {
        ...
        if (side < (-DG_RESTING_CONTACT_PENETRATION))
        {
          ...
          {
            contactOut[count].m_point = soupMatrix.TransformVector(center)
                - contactOut[count].m_normal.Scale(
...
//	for (j = 0; j < data.m_faceCount; j ++) {
  for (dgInt32 j = 0; (j < data.m_faceCount) && (countleft > 0); j++)
  {
    ...
    {
      if ((data.m_faceCount < 8)
          || collision->OOBBTest(mink.m_matrix, polygon, &planeCache))
      {
...
//		indexArray += data.m_faceIndexCount[j];
  ...
  if (count > 1)
  {
    if (data.m_faceCount > 1)
    {
      dgFloat32 dt;
      dt = dgFloat32(
          0.01f) * dgRsqrt (mink.m_localRelVeloc % mink.m_localRelVeloc);
      for (dgInt32 i = 0; i < count; i++)
      {
        dgFloat32 err;
...
//	_ASSERTE (proxy.m_referenceCollision == hullBody->m_collision);
//	_ASSERTE (proxy.m_floatingCollision == soupBody->m_collision);
  _ASSERTE(
      proxy.m_referenceCollision->IsType (dgCollision::dgConvexCollision_RTTI));
  _ASSERTE(
      proxy.m_floatingCollision->IsType (dgCollision::dgCollisionMesh_RTTI));
  ...
  if (proxy.m_continueCollision)
  {
    ...
    else
    {
      ...
      if (mag2 > dgFloat32(0.1f))
      {
        ...
        dgFloat32 maxOmega = (hullOmega % hullOmega) * proxy.m_timestep
            * proxy.m_timestep;
