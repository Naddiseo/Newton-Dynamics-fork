    ...
    dgInt32 archModel, dgFloat32 timestep)
{
  ...
  m_world->m_perfomanceCounters[m_dynamicsBuildSpanningTreeTicks] = dynamicsTime
      - updateTime;
  ...
  if (threadCounts > 1)
  {
    ...
    if (m_world->m_singleIslandMultithreading)
    {
      ...
      while (m_islands
          && (m_islandArray[m_islands - 1].m_jointCount
              >= DG_PARALLEL_JOINT_COUNT))
      {
        m_islands--;

        _ASSERTE(!m_islandArray[m_islands].m_isContinueCollision);
        BuildJacobianMatrixParallel(m_islandArray[m_islands], timestep,
            archModel);
        system.CalculateReactionsForcesParallel(dgInt32(solverMode),
            DG_SOLVER_MAX_ERROR, archModel);
        IntegrateArray(&system.m_bodyArray[1], system.m_bodyCount - 1,
            DG_SOLVER_MAX_ERROR, timestep, 0, true);
      }
    }
    ...
    if (m_islands)
    {
      ...
      for (dgInt32 threadIndex = 0; threadIndex < threadCounts; threadIndex++)
      {
        ...
        m_workerThreads[threadIndex].m_count = chunkSizes[threadIndex]
            * threadCounts;
  ...
  m_world->m_perfomanceCounters[m_dynamicsSolveSpanningTreeTicks] = ticks
      - dynamicsTime;
    ...
    for (dgInt32 i = 0; i < count; i += step)
    {
      ...
      else
      {
        ...
        dgInt32 steps = dgInt32(
            dgFloat32(4.0f) * dgSqrt (maxSpeed2) * m_timestep / dist) + 1;
    ...
    dgInt32 threadIndex)
{
  ...
  if (count)
  {
    ...
    newCount = ((m_world->m_jacobiansMemorySizeInBytes[threadIndex] - 16)
        / stride) & (-8);
    ...
    dgInt32 threadIndex)
{
  ...
  if (count)
  {
    ...
    newCount = ((m_world->m_internalForcesMemorySizeInBytes[threadIndex] - 16)
        / stride) & (-8);
  ...
  while (!queue.IsEmpty())
  {
    ...
    for (dgInt32 j = 0; j < count; j++)
    {
      ...
      for (dgBodyMasterListRow::dgListNode* jointNode =
          body->m_masterNode->GetInfo().GetFirst(); jointNode; jointNode =
          jointNode->GetNext())
      {
...
void dgWorldDynamicUpdate::SpanningTree(dgBody* const body)
{
  ...
  dgQueue<dgBody*> queue(
      ...
      (dgInt32(m_world->m_pairMemoryBufferSizeInBytes >> 1))
          / dgInt32(sizeof(void*)));
...
//isInEquilibrium = 0;
      ...
      bodyCount++;
      for (dgBodyMasterListRow::dgListNode* jointNode =
          srcBody->m_masterNode->GetInfo().GetFirst(); jointNode; jointNode =
          jointNode->GetNext())
      {
        dgBodyMasterListCell& cell = jointNode->GetInfo();
    ...
    dgInt32 threadIndex, bool update) const
{
  ...
  for (dgInt32 i = 0; i < count; i++)
  {
    ...
    if (body->m_invMass.m_w && body->m_isInWorld)
    {
      ...
      bool equilibrium = (accel2 < accelFreeze) && (alpha2 < accelFreeze)
          && (speed2 < speedFreeze) && (omega2 < speedFreeze);
      if (equilibrium)
      {
  ...
  if (update)
  {
    ...
    if (isAutoSleep)
    {
      ...
      {
        if ((maxAccel > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxAccel)
            || (maxAlpha
                > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxAlpha)
            || (maxSpeed
                > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxVeloc)
            || (maxOmega
                > m_world->m_sleepTable[DG_SLEEP_ENTRIES - 1].m_maxOmega))
        {
          for (dgInt32 i = 0; i < count; i++)
          {
            dgBody* const body = bodyArray[i].m_body;
    ...
    dgInt32 threadIndex, bool bitMode, dgInt32 rowCount, dgFloat32 timestep)
{
  ...
  for (dgInt32 j = 0; j < jointCount; j++)
  {
    ...
    if (constraint->m_isUnilateral ^ bitMode)
    {
      ...
      dgInt32 m0 =
          (constraint->m_body0->m_invMass.m_w != dgFloat32(0.0f)) ?
              constraint->m_body0->m_index : 0;
      dgInt32 m1 =
          (constraint->m_body1->m_invMass.m_w != dgFloat32(0.0f)) ?
              constraint->m_body1->m_index : 0;
      ...
      for (dgInt32 i = 0; i < dof; i++)
      {
        ...
        solverMemory.m_normalForceIndex[rowCount] =
            constraintParams.m_forceBounds[i].m_normalIndex
                + ((constraintParams.m_forceBounds[i].m_normalIndex >= 0) ?
                    (rowCount - i) : 0);
      ...
      rowCount =
          (rowCount & (DG_SIMD_WORD_SIZE - 1)) ?
              ((rowCount & (-DG_SIMD_WORD_SIZE)) + DG_SIMD_WORD_SIZE) :
              rowCount;
...
//	dgInt32 threads = m_world->m_numberOfTheads;
  ...
  for (dgInt32 j = 0; j < jointCount; j++)
  {
    ...
    dgInt32 dof =
        dgInt32(
            (constraint->m_maxDOF & (DG_SIMD_WORD_SIZE - 1)) ?
                ((constraint->m_maxDOF & (-DG_SIMD_WORD_SIZE))
                    + DG_SIMD_WORD_SIZE) :
                constraint->m_maxDOF);
    maxRowCount += dof;
  }
    ...
    dgInt32 threadIndex, dgFloat32 timestep)
{
  ...
  for (dgInt32 k = 0; k < jointCount; k++)
  {
    ...
    for (dgInt32 i = 0; i < count; i++)
    {
      ...
      dgFloat32 extenalAcceleration = -(tmpAccel.m_x + tmpAccel.m_y
          + tmpAccel.m_z);
...
//		}
    ...
    hasJointFeeback |= (
        constraintArray[i].m_joint->m_updaFeedbackCallback ? 1 : 0);
    ...
    dgFloat32 maxAccNorm) const
{
  ...
  for (dgInt32 i = 0; i < count; i++)
  {
    ...
    accel[i] = coordenateAccel[i] - acc.m_x - acc.m_y - acc.m_z
        - force[i] * diagDamp[i];
        ...
        passes++)
    {
      ...
      for (dgInt32 curJoint = 0; curJoint < m_jointCount; curJoint++)
      {
        ...
        for (dgInt32 k = 0; k < rowsCount; k++)
        {
          ...
          dgFloat32 a = coordenateAccel[index] - acc.m_x - acc.m_y - acc.m_z
              - force[index] * diagDamp[index];
          ...
          dgFloat32 lowerFrictionForce = frictionNormal
              * lowerFrictionCoef[index];
          dgFloat32 upperFrictionForce = frictionNormal
              * upperFrictionCoef[index];
...
//	ticks = m_world->m_getPerformanceCount();
  ...
  for (dgInt32 j = 0; j < count; j++)
  {
    ...
    if (constraint->m_isUnilateral ^ m_bitMode)
    {
      ...
      m_rowsCount[0] = rowCount
          + ((dof & (DG_SIMD_WORD_SIZE - 1)) ?
              ((dof & (-DG_SIMD_WORD_SIZE)) + DG_SIMD_WORD_SIZE) : dof);
      m_world->dgReleasedUserLock();
...
//			maxForce = GetMax(dgAbsf (val), maxForce);
    ...
    m_hasJointFeeback |= (
        m_constraintArray[i].m_joint->m_updaFeedbackCallback ? 1 : 0);
  ...
  else
  {
    ...
    for (dgInt32 i = 0; i < m_count; i += m_threads)
    {
      ...
      for (dgInt32 k = 0; k < rowsCount; k++)
      {
        ...
        a = m_coordenateAccel[index] - acc.m_x - acc.m_y - acc.m_z
            - m_force[index] * m_diagDamp[index];
...
//		parallelSolverJointAcceleration[threadIndex].m_jointRemapArray = &jointRemapArray[acc];
  ...
  for (dgInt32 threadIndex = 0; threadIndex < threads; threadIndex++)
  {
    ...
    parallelSolverUpdateVeloc[threadIndex].m_internalVeloc = &internalVeloc[acc
        + 1];
...
//accNorm = 1.0f;
  ...
  for (dgInt32 threadIndex = 0; threadIndex < threads; threadIndex++)
  {
    ...
    parallelSolverUpdateForce[threadIndex].m_internalVeloc = &internalVeloc[acc
        + 1];
    acc += chunkBodiesSizes[threadIndex];
    m_world->m_threadsManager.SubmitJob(
