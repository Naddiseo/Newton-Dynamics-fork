...
dgMeshTreeCSGFace::dgMeshTreeCSGFace(const dgMeshEffect& mesh,
    dgEdge* const face) :
    dgList<dgHugeVector>(mesh.GetAllocator()), m_side(
        dgMeshEffectSolidTree::m_divider)
{
...
dgMeshTreeCSGFace::dgMeshTreeCSGFace(dgMemoryAllocator* const allocator,
    dgInt32 count, const dgHugeVector* const points) :
    ...
    const dgHugeVector& p1, const dgHugeVector& q0) const
{
  ...
  {
    if (numf
        < den.GetAproximateValue()
            * dgFloat64(1.0 - (DG_CLIPPER_TOL * dgFloat64(1.0e3))))
    {
      dgGoogol t = num / den;
  ...
  *rightOut = NULL;
  ...
  else
  {
        ...
        ptr = ptr->GetNext())
    {
      ...
      dgHugeVector inter;
      if (((pointSide[i0] == -1) && (pointSide[i1] == 1))
          || ((pointSide[i0] == 1) && (pointSide[i1] == -1)))
      {
...
dgMeshEffectSolidTree::dgMeshEffectSolidTree(dgPlaneType type) :
    m_planeType(type), m_back(NULL), m_front(NULL), m_plane(0.0, 0.0, 0.0, 0.0)
{
}

dgMeshEffectSolidTree::dgMeshEffectSolidTree(const dgMeshEffect& mesh,
    dgEdge* const face) :
    m_planeType(m_divider), m_back(
        new (mesh.GetAllocator()) dgMeshEffectSolidTree(m_solid)), m_front(
        new (mesh.GetAllocator()) dgMeshEffectSolidTree(m_empty)), m_plane(
        BuildPlane(mesh, face))
{
}
...
dgMeshEffectSolidTree::dgMeshEffectSolidTree(const dgHugeVector& plane,
    ...
    m_planeType(m_divider), m_back(
        new (allocator) dgMeshEffectSolidTree(m_solid)), m_front(
        new (allocator) dgMeshEffectSolidTree(m_empty)), m_plane(plane)
{
}
