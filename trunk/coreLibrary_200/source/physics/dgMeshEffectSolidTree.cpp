...
dgMeshTreeCSGFace::dgMeshTreeCSGFace (const dgMeshEffect& mesh, dgEdge* const face)
	:dgList<dgHugeVector>(mesh.GetAllocator()), m_side(dgMeshEffectSolidTree::m_divider)
{
...
dgMeshTreeCSGFace::dgMeshTreeCSGFace (dgMemoryAllocator* const allocator, dgInt32 count, const dgHugeVector* const points)
	:dgList<dgHugeVector>(allocator), m_side(dgMeshEffectSolidTree::m_divider)
{
...
bool dgMeshTreeCSGFace::IsPointOnEdge (const dgHugeVector& p0, const dgHugeVector& p1, const dgHugeVector& q0) const
{
...
dgInt32 dgMeshTreeCSGFace::RemoveDuplicates (dgInt32 count, dgHugeVector* const points) const
{
	dgInt32 index[256];
	for (dgInt32 i = 0;  i < count; i ++) {
		index[i] = i + 1;
...
dgMatrix dgMeshTreeCSGFace::DebugMatrix () const
{
	...
	dgHugeVector e1 (GetFirst()->GetNext()->GetNext()->GetInfo() - GetFirst()->GetInfo());
...
dgHugeVector dgMeshTreeCSGFace::FaceNormal () const
{
	...
	dgHugeVector e0 (GetFirst()->GetNext()->GetInfo() - GetFirst()->GetInfo());
	for (dgListNode* node = GetFirst()->GetNext()->GetNext(); node; node = node->GetNext()) {
...
bool dgMeshTreeCSGFace::CheckFaceArea (dgInt32 count, const dgHugeVector* const points) const
{
	dgHugeVector area (dgFloat64 (0.0), dgFloat64 (0.0), dgFloat64 (0.0), dgFloat64 (0.0));
