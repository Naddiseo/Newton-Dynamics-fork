        ...
        if (ptr->m_incidentFace > 0)
        {
          if (ptr->m_mark != faceMark)
          {
            ...
            do
            {
              ...
              _ASSERTE(
                  faceIndexCount < sizeof (faceIndex) / sizeof (faceIndex[0]));
              faceIndexCount++;
              ptr1 = ptr1->m_next;
            } while (ptr1 != ptr);
            ...
            if (dot < dgFloat64(1.0e-12f))
            {
              ...
              deleteCount++;
              _ASSERTE(
                  deleteCount < sizeof (deleteEdge) / sizeof (deleteEdge[0]));
            }
            else
            {
    ...
    dgInt32 stride)
{
  ...
  for (iter.Begin(); iter; iter++)
  {
    ...
    if (edge->m_incidentFace < 0)
    {
      ...
      do
      {
        ...
        perimeterCount++;
        _ASSERTE(
            perimeterCount < sizeof (edgePerimeters) / sizeof (edgePerimeters[0]));
        ptr = ptr->m_prev;
      } while (ptr != edge);
      break;
    }
  ...
  dgBigVector normal(
      FaceNormal(edgePerimeters[0], vertex,
          dgInt32(stride * sizeof(dgFloat64))));
      ...
      for (iter.Begin(); iter;)
      {
        ...
        if (edge->m_incidentFace > 0)
        {
          ...
          if (flatFace.GetCount())
          {
            ...
            dgInt32 diagonalCount = GetInteriorDiagonals(flatFace,
                diagonalsPool,
                sizeof(diagonalsPool) / sizeof(diagonalsPool[0]));
            if (diagonalCount)
            {
              ...
              do
              {
                ...
                dgBigVector e1(p2 - p1);
                e1 = e1.Scale(
                    dgFloat32(1.0f) / (sqrt(e1 % e1) + dgFloat32(1.0e-14f)));
              ...
              if (edge->m_mark != mark)
              {
                if (edge->m_incidentFace > 0)
                {
                  ...
                  do
                  {
                    ...
                    diagonalCount++;
                    _ASSERTE(
                        diagonalCount < sizeof (polygon) / sizeof (polygon[0]));
                    ptr->m_mark = mark;
                    ptr = ptr->m_next;
                  } while (ptr != edge);
                  if (diagonalCount >= 3)
