...
//#define DG_RAY_TOL_ERROR (dgFloat32 (-1.0e-5f))
#define DG_RAY_TOL_ERROR (dgFloat32 (-1.0e-3f))
//#define DG_RAY_TOL_ERROR (dgFloat32 (-1.0e-2f))
...
//	  m_tolerance (DG_RAY_TOL_ERROR, DG_RAY_TOL_ERROR, DG_RAY_TOL_ERROR, dgFloat32 (0.0f)),
    m_tolerance(DG_RAY_TOL_ERROR, DG_RAY_TOL_ERROR, DG_RAY_TOL_ERROR,
        DG_RAY_TOL_ERROR), m_zero(dgFloat32(0.0f), dgFloat32(0.0f),
...
dgInt32 FastRayTest::BoxTestSimd(const dgVector& minBox,
...
dgInt32 FastRayTest::BoxTest(const dgVector& minBox,
    const dgVector& maxBox) const
{
  dgFloat32 tmin = 0.0f;
  dgFloat32 tmax = 1.0f;
...
dgFloat32 FastRayTest::PolygonIntersectSimd(const dgVector& normal,
...
dgFloat32 FastRayTest::PolygonIntersect(const dgVector& normal,
    const dgFloat32* const polygon, dgInt32 strideInBytes,
    const dgInt32* const indexArray, dgInt32 indexCount) const
{
  _ASSERTE(m_p0.m_w == m_p1.m_w);
